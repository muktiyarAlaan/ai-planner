...
export async function POST(req: Request) {
  ...
  try {
    ...
    // ── Compile all plan data into a structured payload for Gemini ──────────

    const sections: string[] = [];

    sections.push(`PLAN TITLE: ${p.title}`);
    sections.push(`ORIGINAL REQUIREMENT:\n${p.rawRequirement}`);

    const qaContext = p.qaContext as Array<{ question: string; answer: string }> | null;
    if (qaContext?.length) {
    ...
    const requirements = p.requirements as {
      functional?: string[];
    ...
    const userFlows = p.userFlows as {
    ...
    const apiEndpoints = p.apiEndpoints as Array<{
      method: string;
      path: string;
      description: string;
      auth: boolean;
    ...
    const securityReview = p.securityReview as {
    ...
    if (securityReview) {
      ...
      for (const threat of securityReview.threatModel ?? []) {
        secLines.push(`  Threat: ${threat.threat} | Likelihood: ${threat.likelihood} | Impact: ${threat.impact} | Mitigation: ${threat.mitigation}`);
      }
    ...
    const linearTickets = p.linearTickets as Array<{
      title: string;
      type: string;
      description?: string;
      acceptanceCriteria?: string[];
